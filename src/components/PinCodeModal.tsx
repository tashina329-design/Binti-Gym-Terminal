...
import { Lock, Delete, X, AlertCircle, CheckCircle2, KeyRound, UserPlus, UserCheck, ShieldCheck } from 'lucide-react';
...
interface PinCodeModalProps {
  ...
  onSuccess: (authenticatedStaff?: RegisteredStaff) => void;
...
export const PinCodeModal: React.FC<PinCodeModalProps> = ({
  ...
  const handleKeyPress = (digit: string) => {
    ...
    if (newPin.length === 6) {
      ...
      if (foundStaff || isMasterPin) {
        ...
        setErrorMsg('Invalid 6-Digit PIN code. Enter a registered staff PIN.');
        setTimeout(() => {
          setPin('');
        }, 700);
      }
  ...
  const handleRegisterSubmit = (e: React.FormEvent) => {
    ...
    const cleanName = regName.trim() || 'Gym Terminal';
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/85 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-200">
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-7 max-w-sm sm:max-w-md w-full shadow-2xl relative text-slate-100 flex flex-col items-center">
        {/* Close Button */}
        <button
          onClick={onCancel}
        ...
        <h2 className="text-xl font-black text-center text-white tracking-tight">
          Binti Gym Staff Authorization
        </h2>
        <p className="text-xs text-slate-400 text-center mb-4 mt-0.5 font-medium">
          6-Digit PIN Required to Unlock Terminal
        </p>

        {/* Mode Switch Tabs */}
        <div className="flex bg-slate-950 p-1 rounded-xl border border-slate-800 w-full mb-5">
          <button
            type="button"
            onClick={() => {
              setActiveTab('enter_pin');
          ...
          >
            <KeyRound className="w-3.5 h-3.5" /> Unlock Terminal
          </button>
          <button
            type="button"
            onClick={() => {
        ...
        {activeTab === 'enter_pin' && (
          <div className="w-full flex flex-col items-center animate-in fade-in">
            ...
            {/* Error Message */}
            {errorMsg && (
              <div className="mb-4 text-xs font-bold text-rose-300 bg-rose-950/60 border border-rose-800/80 px-3 py-2 rounded-xl flex items-center gap-2 text-center animate-in fade-in max-w-xs">
            ...
            <div className="grid grid-cols-3 gap-2 w-full max-w-[240px] mb-4">
              {['1', '2', '3', '4', '5', '6', '7', '8', '9'].map((digit) => (
                <button
                  ...
                  className="h-12 bg-slate-950 hover:bg-slate-800 active:bg-emerald-500 active:text-slate-950 border border-slate-800 rounded-2xl text-xl font-black text-slate-100 shadow-sm transition-all flex items-center justify-center select-none"
                >
                  {digit}
                </button>
              ))}
              <button
                type="button"
                onClick={handleClear}
                className="h-12 bg-rose-950/40 hover:bg-rose-900/60 active:bg-rose-600 border border-rose-800/50 rounded-2xl text-xs font-extrabold text-rose-300 shadow-sm transition-all flex items-center justify-center select-none"
              >
                CLR
              </button>
              <button
              ...
              <button
                ...
                className="h-12 bg-amber-950/40 hover:bg-amber-900/60 active:bg-amber-500 active:text-slate-950 border border-amber-800/50 rounded-2xl text-xs font-bold text-amber-300 shadow-sm transition-all flex items-center justify-center select-none"
              >
            ...
            <div className="text-[11px] text-slate-500 text-center mt-1">
              {registeredStaff.length > 0 ? (
                ...
                <span>
                  Default 6-Digit PIN: <strong className="text-emerald-400 font-mono">123456</strong>
                </span>
              )}
            </div>
          </div>
          ...
          <form onSubmit={handleRegisterSubmit} className="w-full space-y-3.5 animate-in fade-in">
            ...
            {regSuccess && (
              <div className="text-xs font-bold text-emerald-300 bg-emerald-950/60 border border-emerald-800/80 px-3 py-2 rounded-xl flex items-center gap-2">
                ...
                <span>6-Digit PIN created successfully! Unlocking terminal...</span>
              </div>
            )}

            <div>
              <label className="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">
                Gym / Terminal Name (Optional)
              </label>
              <input
                type="text"
                placeholder="e.g. Main Gym Terminal"
                value={regName}
            ...
            >
              <UserCheck className="w-4 h-4" /> Create & Register 6-Digit PIN
            </button>
          </form>
        )}
      </div>
